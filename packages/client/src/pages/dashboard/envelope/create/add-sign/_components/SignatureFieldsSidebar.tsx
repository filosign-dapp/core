...
import { fieldTypeConfigs, type SignatureField } from "../mock"
...
export default function SignatureFieldsSidebar({ onAddField, isPlacingField, pendingFieldType }: SignatureFieldsSidebarProps) {
    return (
                ...
                <div className="text-xs text-muted-foreground space-y-2">
                    {isPlacingField && (
                        <div className="mt-3 p-2 bg-primary/5 border border-primary/20 rounded text-primary text-xs">
                            <strong>Placing:</strong> {fieldTypes.find(f => f.type === pendingFieldType)?.label}
                        </div>
                    )}
                </div>
            </div>
