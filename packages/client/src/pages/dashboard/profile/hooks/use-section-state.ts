...
const profileSchema = z.object({
    ...
    pin: z.object({
        current: z.string().min(1, "Current PIN is required").regex(/^\d{6}$/, "PIN must be exactly 6 digits"),
        new: z.string().min(1, "New PIN is required").regex(/^\d{6}$/, "PIN must be exactly 6 digits"),
...
export const useSectionState = (sectionKey: SectionKey, form: any, originalValues: ProfileForm) => {
    ...
    useEffect(() => {
        const subscription = form.watch((value: any, { name }: { name?: string }) => {
            ...
            if (sectionKey === 'pin' && name.startsWith('pin.')) {
                ...
                const hasPinChanges = Boolean(pinValues.current || pinValues.new || pinValues.confirm)
                ...
                const hasPersonalChanges = JSON.stringify(personalValues) !== JSON.stringify(original)
                setHasChangesState(hasPersonalChanges)
            } else if (sectionKey === 'preferences' && name.startsWith('preferences.')) {
                const prefValues = form.getValues('preferences')
                const original = originalValues.preferences
                const hasPrefChanges = JSON.stringify(prefValues) !== JSON.stringify(original)
                setHasChangesState(hasPrefChanges)
            } else if (sectionKey === 'profilePicture' && name === 'profilePicture') {
