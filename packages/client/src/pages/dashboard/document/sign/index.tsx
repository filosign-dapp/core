import { useAuthedApi, useFileInfo, useSignFile, useViewFile } from "@filosign/react/hooks";
...
export default function SignDocumentPage() {
    ...
    const { data: file, isLoading: fileLoading, error: fileError } = useFileInfo({
    ...
    const handleViewFile = useCallback(async () => {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : "Failed to load file for signing";
    ...
    useEffect(() => {
        if (file && file.acked && file.kemCiphertext && file.encryptedEncryptionKey && !fileData && !viewFile.isPending) {
            handleViewFile();
        }
    ...
    const renderSignatureIndicator = useCallback(() => {
        ...
        return (
            ...
            >
                <SignatureIcon className={isMobile ? "size-4" : "size-6"} weight="fill" />
                <span className={`font-medium text-primary whitespace-nowrap ${isMobile ? "text-[10px]" : "text-xs"}`}>
                    Click to Sign
                </span>
            </div>
        );
    ...
    if (authLoading || fileLoading) {
        return <Loader text={authLoading ? "Authenticating..." : "Preparing document..."} />;
    }
                    ...
                    >
                        {viewFile.isPending && (
                            <div className="absolute inset-0 flex items-center justify-center bg-background/80 z-20">
                                <Loader text="Loading document..." size="sm" className="min-h-0" />
                            </div>
                        )}
                        {renderFileContent()}
                    </div>
