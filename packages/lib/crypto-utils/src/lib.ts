...
import { type Hex, hexToBytes, keccak256, stringToBytes, toHex } from "viem";
...
type WasmZeroize = (u: BufferSource | null | undefined) => Promise<void>;
...
export function genMod(options: {
	...
	async function hkdfExtractExpand(
		...
		const hkdfKey = await crypto.subtle.importKey(
			...
			ikm,
			"HKDF",
			false, //always not extractable as it will not expose rawbytes now
			["deriveBits"],
		);
	...
	async function generateRandomHex() {
		const buf = generateRandomBytes32();
		const hex = toHex(buf, { size: 32 });
	...
	async function deriveEncryptionMaterial(
		...
		const pinSaltBytes = hexToBytes(pinSaltHex);
		const authSaltBytes = hexToBytes(authSaltHex);
		const wrapperSaltBytes = hexToBytes(wrapperSaltHex);
		const signatureBytes = hexToBytes(signatureHex);

		let pinKey: Uint8Array | undefined;
		try {
			pinKey = await argon2idHash(
				new Uint8Array(stringToBytes(pin)),
				pinSaltBytes,
			);
		} catch (e) {
			throw new Error(`argon2 failed with error ${e}`);
		}

		let authKey: Uint8Array | undefined;
		try {
			authKey = await hkdfExtractExpand(
				authSaltBytes,
				signatureBytes,
				new TextEncoder().encode("auth key"),
				32,
			);
		} catch (e) {
		...
		for (let i = 0; i < 32; i++)
			x[i] = (authKey as any)[i] ^ (pinKey as any)[i];

		let wrapperKey: Uint8Array | null = null;
		try {
			wrapperKey = await hkdfExtractExpand(
				ws.v,
				x,
				new TextEncoder().encode("wrapper key"),
				32,
			);
		} catch (e) {
			await wasmZeroize(authKey);
			await wasmZeroize(pinKey);
			await wasmZeroize(x);
		...
		try {
			...
			await wasmZeroize(pinKey);
			await wasmZeroize(x);
			await wasmZeroize(wrapperKey);
			return errMsg("rng failure");
		}

		const commitBuf = new Uint8Array(
		...
		try {
			encryptionKey = await hkdfExtractExpand(
				null,
				seed,
				infoBytesDecoded.v,
				32,
		...
		await wasmZeroize(encCombined);
		await wasmZeroize(infoBytesDecoded.v);
	...
	async function regenerateEncryptionKey(
		...
		{ ok: true; value: RegenerationResult } | { ok: false; error: string }
	> {
		const ps = hexDecodeChecked(pinSaltHex);
		if (!ps.ok) return errMsg(`pin_salt_hex: ${ps.error}`);
		const asalt = hexDecodeChecked(authSaltHex);
		if (!asalt.ok) return errMsg(`auth_salt_hex: ${asalt.error}`);
		const ws = hexDecodeChecked(wrapperSaltHex);
		if (!ws.ok) return errMsg(`wrapper_salt_hex: ${ws.error}`);
		const sig = hexDecodeChecked(signatureHex);
		if (!sig.ok) return errMsg(`signature_hex: ${sig.error}`);
		const encCombinedDecoded = hexDecodeChecked(encSeedHex);
		if (!encCombinedDecoded.ok)
			return errMsg(`enc_seed_hex: ${encCombinedDecoded.error}`);
		...
		let seed: Uint8Array | null = null;
		try {
			seed = sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(
				null,
				ciphertext,
				null,
				nonce24,
		...
		const infoDecoded = hexDecodeChecked(infoHex);
		if (!infoDecoded.ok) {
			await wasmZeroize(authKey);
			await wasmZeroize(pinKey);
			await wasmZeroize(x);
			await wasmZeroize(wrapperKey);
			await wasmZeroize(seed);
			return errMsg(`info_hex: ${infoDecoded.error}`);
		}
	...
	async function generateKeyPair(): Promise<
		{ ok: true; value: KeyPairResult } | { ok: false; error: string }
	> {
		try {
	...
	async function createSharedKey(
		...
		{ ok: true; value: SharedKeyResult } | { ok: false; error: string }
	> {
		const pkDecoded = hexDecodeChecked(selfPrivateKeyHex);
		if (!pkDecoded.ok)
			return errMsg(`self_private_key_hex: ${pkDecoded.error}`);
		if (pkDecoded.v.length !== 32)
			return errMsg("private key must be 32 bytes");
		const priv = pkDecoded.v;

		const otherDecoded = hexDecodeChecked(otherPublicKeyHex);
		if (!otherDecoded.ok) {
			await wasmZeroize(priv);
			return errMsg(`other_public_key_hex: ${otherDecoded.error}`);
		}
		...
		try {
			...
			if (otherPubFull.length === 33) {
				otherPubUncompressed = p256.Point.fromHex(
					bytesTo0xHex(otherPubFull).slice(2),
				).toBytes(false);
			...
			const sharedSecret =
				shared instanceof Uint8Array ? shared : new Uint8Array(shared);
			const sharedKey = await hkdfExtractExpand(
				null,
				sharedSecret,
				new TextEncoder().encode("shared encryption key"),
				32,
			);
	...
	async function getPublicKeyFromRegenerated(
		...
		{ ok: true; value: PublicKeyResult } | { ok: false; error: string }
	> {
		const regen = await regenerateEncryptionKey(
			signatureHex,
			pin,
			pinSaltHex,
			authSaltHex,
		...
		const ekDecoded = hexDecodeChecked(encryptionKeyHex);
		if (!ekDecoded.ok) return errMsg(`encryption_key hex: ${ekDecoded.error}`);
		if (ekDecoded.v.length !== 32) {
			await wasmZeroize(ekDecoded.v);
			return errMsg("encryption key must be 32 bytes");
		}
