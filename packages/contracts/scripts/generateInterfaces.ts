...
function typeNameToString(node: TypeName): string {
    ...
    switch (node.type) {
        ...
        case "UserDefinedTypeName":
            ...
            return node.name ?? "UnknownType";
        case "ArrayTypeName":
            return `${node.baseTypeName ? typeNameToString(node.baseTypeName) : "UnknownType"}${node.length ? `[${node.length}]` : "[]"
                }`;
        case "Mapping":
            return `mapping(${node.keyType ? typeNameToString(node.keyType) : "UnknownType"} => ${node.valueType ? typeNameToString(node.valueType) : "UnknownType"
                })`;
